...
webpack({}, (err, stats) => {
  if (err || stats.hasErrors()) {
    // ...
>>>>>>> 25a6e2ed5f98da6483591c4704a546a62f67c89c
  }
  // 处理完成
});
```

T> `err`对象 **不包含** 编译错误，必须使用 `stats.hasErrors()` 单独处理，文档的 [错误处理](#error-handling) 将对这部分进行详细介绍。`err` 对象只包含 webpack 相关的问题，例如配置错误等。
